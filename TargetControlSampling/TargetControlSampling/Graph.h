...
public:
	...
	vector<vector<int>> getInLink() const { return in_link; }
	vector<vector<int>> getOutLink() const { return out_link; }
	set<int> getTargetSet() const { return target_set; }
	vector<int> getMatchLeft() const{ return match_left; }
	vector<int> getMatchRight() const{ return match_right; }
...
private:
	...
	void unmatch_removed_node_and_max_matching_new_node(int select_side, int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right);
	void random_max_matching(set<int>& new_target_set, set<int>& driver_set, const vector<vector<int>>& link_table);
	void unmatch_left_node_and_max_matching(int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right);
