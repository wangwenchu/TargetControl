...
public:
	vector<vector<int>> getInLink() { return in_link; }
	vector<vector<int>> getOutLink() { return out_link; }
	set<int> getTargetSet() { return target_set; }
	vector<int> getMatchLeft() { return match_left; }
	vector<int> getMatchRight() { return match_right; }
...
private:
	...
	void unmatch_removed_node_and_max_matching_new_node(int select_side,int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right, const set<int>& cur_target_set);
	void random_max_matching(set<int>& new_target_set, set<int>& driver_set, const vector<vector<int>>& link_table);
	void unmatch_left_node_and_max_matching(int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right, const set<int>& cur_target_set);
	void unmatch_right_node_and_max_matching(int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right);
};
