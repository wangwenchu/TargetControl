...
void Graph::unmatch_removed_node_and_max_matching_new_node(int select_side, int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right, const set<int>& always_match_set, const vector<int>& remain_set)
{
	if (select_side == LEFT) {
		unmatch_left_node_and_max_matching(unmatch_node, new_pick_node, cur_match_left, cur_match_right, always_match_set, remain_set);
	}
	else {
		unmatch_right_node_and_max_matching(unmatch_node, new_pick_node, cur_match_left, cur_match_right, always_match_set, remain_set);
	}
}
...
void Graph::unmatch_left_node_and_max_matching(int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right, const set<int>& always_match_set, const vector<int>& remain_set)
{
...
void Graph::unmatch_right_node_and_max_matching(int unmatch_node, int new_pick_node, vector<int>& cur_match_left, vector<int>& cur_match_right, const set<int>& always_match_set, const vector<int>& remain_set)
{
...
void Graph::mark_for_max_matching_a_node(const set<int>& always_match_set, const vector<int>& remain_set)
{
	std::fill(mark.begin(), mark.end(), 1);
	for (int x : always_match_set) {
		mark[x] = 0;
	}
	for (int x : remain_set) {
		mark[x] = 0;
...
std::vector<int> Graph::target_control_node_sampling(int sample_times)
{
	...
	for (int ix = 1; ix <= sample_times; ++ix) {
		...
		if (right_remain.size() > 0) {
			...
			unmatch_removed_node_and_max_matching_new_node(RIGHT, right_pick_node, new_right_matched_node, cur_match_left, cur_match_right, left_always, left_remain);
		}
...
std::vector<int> Graph::full_contrl_node_sampling(int sample_times)
{
	...
	for (int ix = 1; ix <= sample_times; ++ix) {
		...
		if (right_remain.size() > 0) {
			...
			/*
			* 下面这个匹配过程可以简单些，但为了复用代码，保持一致
			*/
			unmatch_removed_node_and_max_matching_new_node(RIGHT, pick_node_1, new_right_matched_node, cur_match_left, cur_match_right, left_always, left_remain);
		}
